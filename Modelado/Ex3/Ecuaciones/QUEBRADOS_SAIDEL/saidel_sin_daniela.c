...
int main()
{
    ...
    if (LeerSistema(array) == 0)
    {
        ...
        else
        {
            ...
            {
                printf("\nLa matriz no puede tener una Diagonal Pesada.\nEsto puede resultar en soluciones no reales o infinitas.\n");
            }
        }
    }
...
int Validar_Diagonal_Pesada(double array[filas][columnas])
{
    ...
    for (i = 0; i < filas; i++)
    {
        ...
        double diagonal_principal = fabs(array[i][i]);
        ...
        if (suma_alrededor >= diagonal_principal)
            return 0; // No cumple la condición
    }

    return 1; // Cumple la condición
}
